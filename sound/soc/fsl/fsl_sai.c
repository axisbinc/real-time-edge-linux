    ...
    struct snd_soc_dai* cpu_dai)
{
    ...
    dev_info(cpu_dai->dev,
        "SAI hw_params: ch=%u slots=%u slot_width=%u pins=%u dl_cfg_idx=%d mask=0x%x\n",
        channels, slots, slot_width, pins, dl_cfg_idx, dl_cfg[dl_cfg_idx].mask[tx]);
    ...
    if (sai->is_multi_fifo_dma) {
        ...
        dev_info(cpu_dai->dev,
            "SAI multi-FIFO: tx=%d words_per_fifo=%u n_fifos_dst=%u stride=%u maxburst=%u start_off=%u next_off=%u\n",
            tx, sai->audio_config[tx].words_per_fifo,
            tx ? sai->audio_config[tx].n_fifos_dst : sai->audio_config[tx].n_fifos_src,
            tx ? sai->audio_config[tx].stride_fifos_dst : sai->audio_config[tx].stride_fifos_src,
            dma_params->maxburst,
            dl_cfg[dl_cfg_idx].start_off[tx],
            dl_cfg[dl_cfg_idx].next_off[tx]);
    ...
    dev_info(cpu_dai->dev,
        "SAI TRCE: tx=%d mask=0x%x trce_mask=0x%x final_trce=0x%x pins=%u\n",
        tx, dl_cfg[dl_cfg_idx].mask[tx], trce_mask,
        dl_cfg[dl_cfg_idx].mask[tx] & trce_mask, pins);
...
static int fsl_sai_probe(struct platform_device* pdev)
{
    ...
    if (!sai->soc_data->use_edma && !ret && dmas[2] == IMX_DMATYPE_MULTI_SAI) {
        ...
        dev_info(dev, "SAI: Multi-FIFO DMA mode enabled (RX dmas[2]=%u)\n", dmas[2]);
    }
    else {
        dev_info(dev, "SAI: DMA probe: use_edma=%d ret=%d dmas[2]=%u IMX_DMATYPE_MULTI_SAI=%d\n",
            sai->soc_data->use_edma, ret, ret ? 0 : dmas[2], IMX_DMATYPE_MULTI_SAI);
    }
