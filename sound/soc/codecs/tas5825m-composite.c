...
static void tas5825m_refresh(struct tas5825m_priv *tas5825m)
{
	...
	dev_info(&tas5825m->i2c->dev, "refresh: is_muted=%d, vol=%d/%d\n",
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (tas5825m->vol[0] != ucontrol->value.integer.value[0] ||
		...
		tas5825m->vol[1] = ucontrol->value.integer.value[1];
		dev_info(component->dev, "set vol=%d/%d (is_powered=%d)\n",
...
static void tas5825m_init(struct tas5825m_priv *tas5825m)
{
	...
	regmap_write(rm, REG_DEVICE_CTRL_2, 0x03); // Play mode
	regmap_write(rm, REG_FAULT, 0x80); // Clear fault
	usleep_range(10000, 20000);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	if (event & SND_SOC_DAPM_PRE_PMD) {
		...
		dev_info(component->dev, "DSP shutdown\n");
		...
		if (tas5825m->is_powered) {
			...
			dev_info(component->dev, "fault regs: CHAN=%02x, "
...
static int tas5825m_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	...
	mutex_lock(&tas5825m->lock);
	dev_info(component->dev, "set mute=%d (is_powered=%d)\n",
