...
/*!
 ...
 @param [in]     nAddrwidth   Data unpacking boundary (1/2/4 byte)
...
static void adi_a2b_NetworkSetup(struct device *dev)
{
    ...
    unsigned int nIndex, nIndex1, nDelayVal;
    ...
    for (nIndex = 0; nIndex < CONFIG_LEN; nIndex++) {
        ...
        case WRITE:
            adi_a2b_Concat_Addr_Data(aDataBuffer,
                                     pOPUnit->nAddrWidth,
                                     pOPUnit->nAddr);
            memcpy(&aDataBuffer[pOPUnit->nAddrWidth],
                   pOPUnit->paConfigData,
                   pOPUnit->nDataCount);
            ...
            break;
        case READ:
            memset(aDataBuffer, 0u, pOPUnit->nDataCount);
            adi_a2b_Concat_Addr_Data(aDataWriteReadBuf,
                                     pOPUnit->nAddrWidth,
                                     pOPUnit->nAddr);
            /* Couple of milliseconds should be OK */
            mdelay(2);
            break;
        case DELAY:
            nDelayVal = 0u;
            for (nIndex1 = 0u; nIndex1 < pOPUnit->nDataCount; nIndex1++) {
                nDelayVal = (pOPUnit->paConfigData[nIndex1]
                             | (nDelayVal << 8u));
            }
            mdelay(nDelayVal);
            break;
...
static int a2b24xx_mute(struct snd_soc_dai *dai,
                        int mute,
                        int stream)
{
...
static int a2b24xx_set_dai_fmt(struct snd_soc_dai *dai,
                               unsigned int fmt)
{
    struct a2b24xx *a2b24xx =
        snd_soc_component_get_drvdata(dai->component);
...
/* Check system clock */
static bool a2b24xx_check_sysclk(unsigned int mclk,
...
/* driver probe */
int a2b24xx_probe(struct device *dev,
                  struct regmap *regmap,
                  enum a2b24xx_type type,
                  void (*switch_mode)(struct device *dev))
{
    ...
    return snd_soc_register_component(dev,
                                       &a2b24xx_codec_driver,
                                       &a2b24xx_dai,
                                       1);
}
EXPORT_SYMBOL_GPL(a2b24xx_probe);

static bool a2b24xx_register_volatile(struct device *dev,
                                      unsigned int reg)
{
    return true;
}
