...
static int sdma_load_context(struct sdma_channel* sdmac)
{
    ...
    dev_info(sdma->dev, "SDMA ch%d load_context: script_addr=%d dir=%s\n",
        channel, load_address,
        sdmac->direction == DMA_DEV_TO_MEM ? "DEV_TO_MEM" :
        sdmac->direction == DMA_MEM_TO_DEV ? "MEM_TO_DEV" :
        sdmac->direction == DMA_DEV_TO_DEV ? "DEV_TO_DEV" : "MEM_TO_MEM");
    dev_info(sdma->dev, "SDMA ch%d wml=0x%08lx shp=0x%08x per=0x%08x\n",
        channel, sdmac->watermark_level, sdmac->shp_addr, sdmac->per_addr);
    dev_info(sdma->dev, "SDMA ch%d event_mask0=0x%08x event_mask1=0x%08x\n",
        channel, (u32)sdmac->event_mask[0], (u32)sdmac->event_mask[1]);
...
static void sdma_set_watermarklevel_for_sais(struct sdma_channel* sdmac)
{
    ...
    dev_info(sdma->dev,
        "SDMA ch%d multi-SAI: direction=%s n_fifos=%u stride_fifos=%u words_per_fifo=%u\n",
        sdmac->channel,
        sdmac->direction == DMA_DEV_TO_MEM ? "RX" : "TX",
        n_fifos, stride_fifos, words_per_fifo);
    ...
    dev_info(sdma->dev,
        "SDMA ch%d watermark_level=0x%08lx (N_FIFOS=%u OFF_FIFOS=%u WORDS=%u)\n",
        sdmac->channel, sdmac->watermark_level, n_fifos, stride_fifos,
        words_per_fifo ? (words_per_fifo - 1) : 0);
}
